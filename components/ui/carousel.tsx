    ...
    return (
      <div ref={ref} className={`flex justify-center ${props.className}`}>
        {Array.from({ length: numberOfSlides }, (_, i) => (
          <Button
            key={i}
            className={`mx-[5px] h-2.5 w-2.5 rounded-full p-0 ${
              i === currentSlide
                ? "scale-125 transform bg-primary hover:bg-primary"
                : "bg-[#F7F7F7] hover:bg-[#F7F7F7]"
            }`}
...
>(({ className, variant = "outline", size = "icon", ...props }, ref) => {
  ...
  return (
    <Button
      ...
      size={size}
      className={cn(
        "w-fit px-2.5 py-3 h-fit disabled:bg-transparent",
        className
      )}
...
const CustomPrev = React.forwardRef<
  HTMLButtonElement,
  React.ComponentProps<typeof Button>
>(({ className, variant = "outline", size = "icon", ...props }, ref) => {
